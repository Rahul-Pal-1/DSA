// using map and vector find frequency sort
#include <bits/stdc++.h>
using namespace std;
...
{
	//hash contain key and index and count of key in a pair;
	unordered_map<int, pair<int, int> > hash; // hash map
	for (int i = 0; i < n; i++) {
		if (hash.find(arr[i]) != hash.end()) //checks wheather element is present or not since find(val) return end() if val is not present
			hash[arr[i]].second++;
		else
			hash[arr[i]] = make_pair(i, 1); //inserting element from array to map
	...
	// cout<<endl<<" loop for check purpose"<<endl;
	// for(auto i : b){
